...
Tests should not be uniformly organized by the class under test. The organization must reflect the scope of the test.

For unit tests, organizing by the tested class is recommended, as each test suite focuses on a single unit of behavior.
For interaction tests, organization may follow the primary component under test, especially when verifying its collaborations with dependencies.
For integration and end-to-end tests, organizing strictly by class is discouraged. These tests should be structured around capabilities, technical boundaries, or business flows (such as persistence, messaging, observability, or API behavior), as they validate behavior across multiple components.

The guiding principle is that test organization should make the intent of the test immediately clear: what behavior or flow is being validated, rather than which class happens to be involved.
