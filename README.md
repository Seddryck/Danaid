...
```bash
dotnet tool install -g Danaid-cli
```
...
```bash
dotnet tool update -g Danaid-cli
```
...
```powershell
docker pull seddryck/danaid:latest
```

#### Running Danaid from Docker

Once you have the Docker image, you can run Danaid using Docker in PowerShell.

###### Basic Command

<sub>CMD:</sub>
```CMD
docker run --rm -v %cd%:/files danaid -t <template-file> -s <source-file> -o <output-file>
```

<sub>PowerShell:</sub>
```powershell
docker run --rm -v ${pwd}:/files danaid -t <template-file> -s <source-file> -o <output-file>
```

- `--rm`: Automatically removes the container after it finishes executing.
- `-v ${pwd}:/files`: Mounts the current directory (`${pwd}` in PowerShell or Bash, `%cd%` in CMD) to /files inside the Docker container, so Danaid can access your local files.
...
- `-o <output-file>`: Specifies the path to the output file that Danaid will generate. If omitted, it will display the result on the host console.
...
1. Prepare the Template and Source Files:
  ...
  - Make sure your template and source files are correctly formatted and saved in the correct directory. For example:
    ...
    - `./data/data.json`
2. Run Danaid: Use the following command to generate templated output:

```powershell
docker run --rm -v ${pwd}:/files danaid -t /files/templates/template-01.hbs -s /files/data/data.json -o /files/output/output.txt
```
...
To update to the latest version of Danaid, either pull the new Docker image
...
Example:
   ...
   ```
   https://github.com/Seddryck/Danaid/releases/latest/
   ```

#### Step 2: Extract the ZIP File

1. Right-click the downloaded `.zip` file and choose **Extract All**.
2. Extract the contents to a directory of your choice, such as `C:\Program Files\Danaid`.
...
Danaid utilizes some templating engines, which allow for powerful and flexible templating.
...
The command to run Danaid is simply `danaid`. When executing it, you need to provide three required arguments:
...
```bash
danaid -t template.scriban -s data.yaml -o page.html
```
...
- Accept: single value. When omitted Danaid will select the engine based on the extension of the template file.
...
- Accept: single value.
- Mandatory: no expect if `--stdin` is specified. When omitted Danaid will select the parser based on the extension of the source file
...
<sub>CMD:</sub>
```cmd
type "data.json" | danaid --stdin -t template.hbs -r json
```

<sub>PowerShell:</sub>
```powershell
Get-Content data.json | danaid --stdin -t template.hbs -r json
```

<sub>Bash:</sub>
```bash
cat data.json | danaid --stdin -t template.hbs -r json
```
