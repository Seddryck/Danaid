...
1. Navigate to the **GitHub repository** of [Danaid](https://github.com/Seddryck/Danaid).
...
```bash
wget https://github.com/Seddryck/Danaid/releases/download/v0.13.0/Danaid-0.13.0-net7.0-linux-x64.zip
```

## Step 2: Extract the ZIP File

1. Open a terminal and navigate to the directory where you downloaded the `.zip` file.
2. Use the `unzip` command to extract the contents. You can choose any directory for extraction (e.g., `/usr/local/bin/Danaid`).

```bash
unzip danaid.zip -d /usr/local/bin/Danaid
```
...
2. Add the following line at the end of the file to add the Danaid directory to your PATH:

```bash
export PATH="/usr/local/bin/Danaid:$PATH"
```
...
2. Type the name of the executable (e.g., `danaid`) and hit Enter.
