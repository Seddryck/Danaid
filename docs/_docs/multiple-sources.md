---
title: Using Multiple Sources in Danaid
tags: [cli-usage]
---
Danaid supports multiple data sources directly from the console, allowing you to build complex models by defining each source as a distinct part. Each source is specified using a key:value format, where each key becomes a unique child within the model structure, allowing you to reference data in a hierarchical way (e.g., model.key).
...
```powershell
danaid -t template.html -s user:user.yaml;settings:config.json -o output.html
```
...
With this approach, Danaid provides flexibility in managing multi-source data, making it easy to build robust, organized models for dynamic template generation.
