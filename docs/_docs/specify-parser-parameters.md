...
```bash
danaid -t template.hbs -s data.tsv -X tsv:csv -P tsv@delimiter:Semicolumn;tsv@commentChar:#
```
...
The following parameters are accepted by Danaid to define the behavior of a CSV parser:
...
To avoid conflicts with other parts of the command line, Danaid supports the following synonyms for parameter values:
