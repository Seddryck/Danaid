...
<sub>CMD:</sub>
```bash
type "data.json" | danaid --stdin -t template.hbs -r json -o output.txt
```

<sub>PowerShell:</sub>
```powershell
Get-Content data.json | danaid --stdin -t template.hbs -r json -o output.txt
```

<sub>Bash:</sub>
```bash
cat data.json | danaid --stdin -t template.hbs -r json -o output.txt
```
...
<sub>PowerShell:</sub>
```powershell
danaid --stdin -t template.hbs -r json
```
